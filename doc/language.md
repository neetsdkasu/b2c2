...
```
Argument
    ByVal xFlag As Boolean From GR1
    ByRef someNumber As Integer From GR2
    ByRef message As String From GR3,GR4
    ByVal elements(10) As Integer From GR5
End Argument
```
| 書式                                                | 受け取り | 型                 |
|:----------------------------------------------------|:--------:|:------------------:|
...
#### 値渡し・整数
 - `ByVal 引数名 As Intege From レジスタ名`
...
#### 値渡し・整数の固定長配列
 - `ByVal 引数名(上界値) As Intege From レジスタ名`
 - レジスタには整数の固定長配列のデータが格納されているメモリの先頭アドレスが格納されているとみなす
